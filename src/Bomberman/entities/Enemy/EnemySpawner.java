...
public class EnemySpawner extends Enemy {
     ...
     */
    public EnemySpawner(int x, int y, Image img) {
        super(x, y, img);
        throughBrick = false;
        mobDead = Sprite.custom_dead1;
        mobLeft = Sprite.custom_left1;
        mobRight = Sprite.custom_right1;
    }
    ...
    void dying() {
        super.dying();
        EnemyThroughWall enemy = new EnemyThroughWall(x, y, Sprite.custom_left0[0].getFxImage());
        BombermanGame.getGameplay().getAnimations().add(enemy);
    }
