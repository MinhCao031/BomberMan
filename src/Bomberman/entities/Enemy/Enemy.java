...
public abstract class Enemy extends Entity {
     ...
     */
    final int fadingTime = 1000, timeAnimate = 200;
    /**
     ...
     * khả năng của địch.
     */
    boolean throughBrick, getStuck, aiActivated;
    /**
     * true nếu còn sống.
     */
    boolean alive = true;
    ...
    private boolean notEncounterObstacle(String inDirection) {
        switch (inDirection) {
            case "left":{
                ...
                if(y - nextTileYUp < delta && X.howMighty(nextTileUp) < (throughBrick? 2: 1) && X.howMighty(nextTileDown) > (throughBrick? 1: 0) ) {
                    y = nextTileYUp;
                    //x -= speed;
                    //System.out.println(x + " " + y);
                    return true;
                } else if (nextTileYDown - y < delta && X.howMighty(nextTileDown) < (throughBrick? 2: 1) && X.howMighty(nextTileUp) > (throughBrick? 1: 0) ) {
                    y = nextTileYDown;
                    //x -= speed;
                    //System.out.println(x + " " + y);
                    return true;
                }
                if(X.howMighty(nextTileDown) > (throughBrick? 1: 0) || X.howMighty(nextTileUp) > (throughBrick? 1: 0) ) {
                    x = nextTileX + 1;
                    if (aiActivated) getStuck = true;
            ...
            case "up":{
                ...
                if(x - nextTileXUp < delta && X.howMighty(nextTileUp) < (throughBrick? 2: 1) && X.howMighty(nextTileDown) > (throughBrick? 1: 0) ) {
                    x = nextTileXUp;
                    //y -= speed;
                    //System.out.println(x + " " + y);
                    return true;
                } else if (nextTileXDown - x < delta && X.howMighty(nextTileDown) < (throughBrick? 2: 1) && X.howMighty(nextTileUp) > (throughBrick? 1: 0) ) {
                    x = nextTileXDown;
                    //y -= speed;
                    //System.out.println(x + " " + y);
                    return true;
    ...
    void dying() {
        ...
        if (numOfEnemy == 0) {
            S.SE_portalOpen.stop();
            S.SE_portalOpen.play();
        }
    }
